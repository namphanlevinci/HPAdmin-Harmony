	...
	render() {
		...
		if (productList) {
			if (this.state.search) {
				productList = productList.filter((e) => {
					if (e !== null) {
						return (
							e.name.trim().toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1 ||
							e.categoryName.trim().toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1
						);
					}
					return null;
				});
		...
		const columns = [
			...
			},
			{
				...
				width: 150,
				Cell: (row) => {
					const image = row.original.imageUrl !== '' ? row.original.imageUrl : defaultImage;
					return (
						<div
							style={{
					...
					return (
						<div style={{ textAlign: 'center' }}>
							...
							<span style={{ paddingLeft: '20px' }}>
								<GoFile size={20} onClick={() => this.viewDetail(row.original)} />
							</span>
						</div>
					);
				}
		...
		return (
			<div className="content GeneralContent react-transition swipe-up Staff">
				<div className="MerList" style={{ padding: '10px' }}>
					...
					>
						<DialogContent>
							<ProductAdd closePopup={() => this.setState({ isPopupAddProduct: false })} />
						</DialogContent>
					</Dialog>
					...
					>
						<DialogTitle id="alert-dialog-title">{'Archive this product ?'}</DialogTitle>
						<DialogContent>
							<DialogContentText id="alert-dialog-description">
								This product will not appear on the app. You can restore this product by clicking the
								Restore button.
							</DialogContentText>
						</DialogContent>
						<DialogActions>
							<Button onClick={() => this.setState({ dialog: false, productId: '' })} color="primary">
								Disagree
							</Button>
							<Button
								onClick={() => [
					...
					>
						...
						<DialogContent>
							<DialogContentText id="alert-dialog-description">
								This product will appear on the app as well as the related lists.
							</DialogContentText>
						</DialogContent>
						<DialogActions>
							<Button
								onClick={() => this.setState({ restoreDialog: false, productId: '' })}
								color="primary"
							>
								Disagree
							</Button>
