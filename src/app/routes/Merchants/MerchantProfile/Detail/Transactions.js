...
class Transactions extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('https://api2.levincidemo.com/api/CustomerMerchantTransaction/' + ID, { headers: {"Authorization" : `Bearer ${this.props.InfoUser_Login.User.token}`} })
        .then((res) => {
    ...
    render() { 
        ...
        const renderConsumer = ConsumerName.map((e) => {
            const name = e.customer !== null ? e.customer.firstName + ' ' + e.customer.lastName : null
            return (
                <option key={e.customer.customerId} value={name}>{name} </option>
            )
        })
        const renderContent = renderTable.map(e => {
            return (
                <tr key={e.customerMerchantTransactionId}>
                    ...
                    <td>{e.customer !== null ? e.customer.firstName + ' ' + e.customer.lastName : null}</td>
