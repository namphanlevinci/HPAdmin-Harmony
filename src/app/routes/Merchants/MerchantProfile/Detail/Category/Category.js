...
class Category extends Component {
	...
	render() {
		...
		if (cagetoryList) {
			if (this.state.search) {
				cagetoryList = cagetoryList.filter((e) => {
					if (e !== null) {
						return e.name.trim().toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1;
					}
					return null;
				});
			}
					...
					return (
						<div style={{ textAlign: 'center' }}>
							...
							<span style={{ paddingLeft: '20px' }}>
								<FaRegEdit size={20} onClick={() => this.handleEdit(row.original)} />
							</span>
						</div>
					);
				}
		...
		const categorySelect = [ { value: 'Product', label: 'Product' }, { value: 'Service', label: 'Service' } ];
		return (
			<div className="react-transition swipe-up category-container">
				<div style={{ padding: '10px' }}>
					<div style={{ display: 'flex', justifyContent: 'space-between' }}>
						...
						<div>
							{/* NEW ADD FORM */}
							<Button className="btn add-category" onClick={() => this.setState({ cateDialog: true })}>
								NEW CATEGORY
							</Button>
							<Dialog
                          ...
                          console.log({body})
													axios
														.post(
															...
															{
																headers: {
																	Authorization: `Bearer ${this.props.InfoUser_Login
																		.User.token}`
																}
															}
														)
													...
													<form onSubmit={handleSubmit}>
														...
														<div className="select-category">
															<Select
																...
																onChange={(selectedOption) => {
																	...
																	setFieldValue('categoryType', selectedOption.value);
																}}
															/>
														</div>
														{errors.categoryType &&
														touched.categoryType && (
															<div className="input-feedback">{errors.categoryType}</div>
														)}
														...
														<div className="category-button">
															...
															<Button
																style={{ marginTop: '20px' }}
																className="red"
																onClick={() => this.setState({ cateDialog: false })}
															>
																CANCEL
															</Button>
														</div>
					...
					<div className="MListContainer">
						...
						<Dialog
							...
							aria-describedby="alert-dialog-description"
						>
							<DialogTitle id="alert-dialog-title">{'Archive this category ?'}</DialogTitle>
							<DialogContent>
								<DialogContentText id="alert-dialog-description">
									This category will not appear on the app. You can restore this category by clicking
									the Restore button.
								</DialogContentText>
							</DialogContent>
							<DialogActions>
								<Button
									onClick={() => this.setState({ dialog: false, categoryId: '' })}
									color="primary"
								>
									Disagree
								</Button>
						...
						<Dialog
							...
							aria-describedby="alert-dialog-description"
						>
							<DialogTitle id="alert-dialog-title">{'Restore this category ?'}</DialogTitle>
							<DialogContent>
								<DialogContentText id="alert-dialog-description">
							...
							<DialogActions>
								<Button
									onClick={() => this.setState({ restoreDialog: false, categoryId: '' })}
									color="primary"
								>
									Disagree
								</Button>
